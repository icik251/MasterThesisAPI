...
class HtmlDocument(Document):
    ...
    def prepare_text(self):
        ...
        html_text = re.sub("(<small>|</small>)", "", html_text, flags=re.IGNORECASE)
        ...
        tables_generator = (
            s for s in soup.find_all("table") if self.should_remove_table(s)
        )
        ...
        if USE_HTML2TEXT:
            ...
            self.plaintext = h.handle(
                str(soup)
            )  # use soup instead of the original html: it's faster and it benefits from the tables being excluded
        else:
            # paragraphs_analysis = []
            # p_idx = 0
            # has_href = False
                ...
                for s in item_search:
                    if isinstance(s, tuple):
                        ...
                        self.log_cache.append(
                            ("ERROR", "Groups found in Regex, please correct")
                        )
    ...
    def should_remove_table(self, html):
        ...
        if html.stripped_strings:
            ...
            self.log_cache.append(
                ("ERROR", "the should_remove_table function is broken")
            )
...
def is_line_break(e):
    ...
    is_block_tag = e.name != None and e.name in [
        "p",
        "div",
        "br",
        "hr",
