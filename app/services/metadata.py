...
from services.utils import requests_get
...
class Metadata(object):
    def __init__(self, index_url=None):
        ...
        if index_url:
            ...
            for i in infoheads:
                j = i.next_element
                while not (isinstance(j, Tag)) or not ("info") in j.attrs["class"]:
                    j = j.next_element
                # remove colons, spaces, hyphens from dates/times
                if type(j.string) is NavigableString:
                    index_metadata[i.string] = re.sub("[: -]", "", j.string).strip()
            ...
            while not (isinstance(i, NavigableString)) or not (
                re.search("\d{10}", i.string)
            ):
                i = i.next_element
            index_metadata["CIK"] = re.search("\d{5,}", i).group()

            for pair in [
                ["Period of Report", "sec_period_of_report"],
    ...
    def add_data_from_filing_text(self, text):
        ...
        """
        for pair in [
            ["CONFORMED PERIOD OF REPORT:", "sec_period_of_report"],
    ...
    def save_to_json(self, file_path):
        ...
        with open(file_path, "w", encoding="utf-8") as json_output:
            ...
            excerpt_as_json = json.dumps(
                self, default=lambda o: o.__dict__, sort_keys=False, indent=4
            )
            json_output.write(bytes(excerpt_as_json, "utf-8").decode("unicode_escape"))
